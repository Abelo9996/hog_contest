...
PLAYER_NAME = 'Abel Yagubyan' # Change this line!
...
def is_swap(player_score, opponent_score):
    ...
    while player_score >= 10:  
        player_score //= 10
    first_digit , second_digit = player_score , opponent_score%10
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    numofdice, max_roll, max_numofdice = 1, 0, 0
    while numofdice <= 10:
        average_roll_per_dice = make_averaged(roll_dice, num_samples)
        average = average_roll_per_dice(numofdice, dice)
        if average > max_roll:
            max_roll , max_numofdice = average , numofdice
        numofdice += 1
...
def ways_score(k, n):
    ...
    total = 0
    for i in range(2, 7):
        total += ways_score(k - i, n - 1)
    return total


def scoring_probability(k, n):
    if k == 1:
        return 1 - (pow(5, n) / pow(6, n))
    return ways_score(k, n) / pow(6, n)


def win_end_probability(score, opponent_score):
    ...
    if score >= 100:
        ...
        return 0
    opponent_num_rolls = optimal_roll(opponent_score, score)
    probability_of_opponent_winning = roll_probability(
            opponent_num_rolls, opponent_score, score)
    return 1 - probability_of_opponent_winning


def roll_probability(n, score, opponent_score):
    probability_of_winning = 0
    if n == 0:
        probability_of_winning = win_end_probability(
                score + free_bacon(opponent_score), opponent_score)
    else:
        for possible_score in range(1, 6 * n + 1):
            probability_of_winning += scoring_probability(possible_score, n) * win_end_probability(
                    score + possible_score, opponent_score)
    return probability_of_winning


def optimal_roll(score, opponent_score):

    score, opponent_score = score , opponent_score
    best_probability, best_n = 0, 1

    for n in range(0, 11):
        probability = roll_probability(n, score, opponent_score)
        if n <= 4:
            probability *= increase_value
        if probability > best_probability:
            best_probability, best_n = probability, n
    return best_n

def final_strategy(score, opponent_score):
    return optimal_roll(score, opponent_score)
